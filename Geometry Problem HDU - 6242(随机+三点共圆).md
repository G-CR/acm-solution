题意：在一个二维平面中给出n个点，让你找到一个圆心和一个半径，使得 $\lceil \frac{n}{2} \rceil$ 个点在这个圆上。保证答案一定存在。

思路：可以知道三点确定一个圆，我们可以通过枚举三个点然后判断当前圆是否符合要求。思路是这个思路，但是1e5个点不允许这么做。仔细想一想答案一定存在的情况下我从n个点中取一个出来至少有 $\frac{1}{2}$ 的概率可以选中这个圆上的点。所以我通过随机的方法来选出三个点，每一次选三个点在圆上的概率就是 $\frac{1}{8}$ ，循环次数不需要很多次就可以找得到答案了。还有就是需要讨论 n = 1, n = 2的时候，直接选择一个点在圆上就可以了，对于 n = 3, n = 4 的情况，令随便两个点作为直径搞一下就可以了。
