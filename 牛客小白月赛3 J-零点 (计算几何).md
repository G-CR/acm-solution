[题目链接](https://ac.nowcoder.com/acm/problem/15751)

思路：中文题目，题面很清晰。这个题目有几个讨论点需要注意一下。

1、如果两端的射线至少一条与x轴重合了，那么铁定是无穷多个零点了，直接输出-1

2、两端射线单独讨论：如果定义射线的两个端点的纵坐标不同号，那么一定跟x轴有交点

3、讨论中间的线段：如果线段与x轴重合，那么就从左端点遍历到右端点每个加入答案；如果相交就把交点横坐标加入答案

4、我使用set来存答案（自动排序+去重），当size > 3e5 的时候就直接输出-1

5、注意判断交点是否为整数，用强制转换后的数与原数作比较

6、虽然坐标没有超过int值，但是交点会超，记得long long
